...
def load_env_variables() -> Tuple[Optional[str], Optional[str]]:
    ...
    if not username or not password:
        logger.error("Environment variables USERNAME or PASSWORD are not set")
        return None, None
...
def login_to_oddsportal(page: Page, username: str, password: str) -> bool:
    """Log in to OddsPortal using the provided credentials."""
    try:
        logger.info("Navigating to OddsPortal homepage...")
        page.goto("https://www.oddsportal.com/", timeout=60000)
...
def main() -> None:
    ...
    try:
        ...
        if login_to_oddsportal(page, username, password):
            html = navigate_to_value_bets(page)
            if html:
                df = extract_data_from_html(html)
                df = clean_and_process_data(df)
                logger.info("Exporting data to CSV")
                df.to_csv("oddsportal_data_with_login.csv", index=False)
